# 🛡️ Cybersecurity Training Diary

This repository contains my daily learning progress during my cybersecurity training. It includes the concepts I learned, practical activities, tools used, and screenshots of my work.

# 📅 Day 1 – 25-06-2026

## Topics Covered
- Introduction to Cybersecurity
- Introduction to TryHackMe

## What I Learned
- Started my cybersecurity training.
- Created a TryHackMe account.
- Explored the TryHackMe platform.
- Understood how cybersecurity learning paths are organized.
- Learned the importance of hands-on practice in cybersecurity.

<img width="1920" height="1080" alt="Screenshot 2026-07-17 111052" src="https://github.com/user-attachments/assets/0db28b84-93d9-4b9b-8503-2a7732e4ea79" />
<img width="1920" height="1080" alt="Screenshot 2026-07-17 111104" src="https://github.com/user-attachments/assets/cf0723dc-e437-4e67-9ff3-81f28d46abf2" />


# 📅 Day 2 – 26-06-2026

## Topics Covered
- TryHackMe Platform

## What I Learned
- Explored different beginner learning paths.
- Learned how to navigate through rooms and tasks.
- Understood the room structure and progress tracking.
- Read about different cybersecurity domains available on TryHackMe.

## Skills Gained
- Platform Navigation
- Learning Path Exploration
- Basic Cybersecurity Knowledge

<img width="1089" height="771" alt="image" src="https://github.com/user-attachments/assets/f1978f5c-fc81-49cb-905f-43aaa9d22ece" />


# 📅 Day 3 – 27-06-2026

## Topics Covered
- Beginner Practice on TryHackMe

## What I Learned
- Continued exploring beginner rooms.
- Learned how to complete tasks and answer questions.
- Understood the importance of practical learning.
- Improved my understanding of the platform.

## Skills Gained
- Practical Learning
- Task Solving
- Cybersecurity Fundamentals

<img width="1800" height="870" alt="Screenshot 2026-07-17 111518" src="https://github.com/user-attachments/assets/d2799aec-d986-4699-b647-a0902ba94178" />


# 📅 Day 4 – 28-06-2026

## Topics Covered
- Searchlight Room
- Introduction to OSINT

## What I Learned
- Started the Searchlight room on TryHackMe.
- Learned the basics of Open Source Intelligence (OSINT).
- Practiced gathering publicly available information using Google.
- Understood how investigators collect information during cybersecurity investigations.

## Skills Gained
- OSINT Basics
- Information Gathering
- Research Skills
<img width="1582" height="781" alt="Screenshot 2026-07-17 111739" src="https://github.com/user-attachments/assets/10a1ff90-8a31-4acc-aa00-c66a9e7bc95e" />

<img width="1732" height="907" alt="Screenshot 2026-07-17 111625" src="https://github.com/user-attachments/assets/1fc424b0-2deb-46c7-9b51-ab4684ea563e" />


# 📅 Day 5 – 29-06-2026

## Topics Covered
- Searchlight Room Completion

## What I Learned
- Successfully completed the Searchlight room.
- Improved my investigation and research skills.
- Practiced collecting information from public sources.
- Shared my achievement on LinkedIn.

## Skills Gained
- Investigation Techniques
- Google Searching
- Open Source Intelligence




<img width="1582" height="781" alt="Screenshot 2026-07-17 111739" src="https://github.com/user-attachments/assets/2273bd7f-8ba6-4dd4-9fe1-ec02fe5f2b72" />
<img width="1732" height="907" alt="Screenshot 2026-07-17 111625" src="https://github.com/user-attachments/assets/068e2c86-fe58-4685-a57a-314d22c91e0a" />


# 📅 Day 6 – 30-06-2026

## Topics Covered
- OSINT Revision

## What I Learned
- Revised the concepts learned in the Searchlight room.
- Practiced information gathering using search engines.
- Reviewed investigation techniques and OSINT methodology.
- Strengthened my understanding of open-source research.

## Skills Gained
- Information Gathering
- Research Techniques
- Critical Thinking


#####using google,yandax,tineye
<img width="1212" height="481" alt="Screenshot 2026-07-20 091117" src="https://github.com/user-attachments/assets/7ebf069f-d535-494f-88c3-5c45b3c462cc" />

<img width="1398" height="572" alt="Screenshot 2026-07-20 091237" src="https://github.com/user-attachments/assets/805b4433-d801-4995-8b7b-544acf35fb19" />

<img width="1697" height="891" alt="Screenshot 2026-07-20 091451" src="https://github.com/user-attachments/assets/61aa6b83-69bd-410e-9911-948508650e56" />


# 📅 Day 7 – 01-07-2026

## Topics Covered
- VirtualBox Installation


<img width="1920" height="1080" alt="Screenshot 2026-02-08 174508" src="https://github.com/user-attachments/assets/dd4e26c5-ccf9-4e17-aef1-7e8fa48d8a2a" />



  
- Kali Linux Installation



<img width="1920" height="1080" alt="Screenshot 2026-07-20 101436" src="https://github.com/user-attachments/assets/92942fe1-32a1-4d30-832d-a0e8ceaca839" />



- Linux Terminal Basics



## What I Learned
- Installed VirtualBox.
- Installed Kali Linux successfully.
- Learned why Kali Linux is used in ethical hacking and penetration testing.
- Opened the Linux terminal and became familiar with its interface.

## Commands Practiced

```bash
pwd
ls
cd
mkdir
touch
cat
find
grep
```

## Skills Gained
- Kali Linux Setup
- Linux Basics
- Terminal Navigation


# 📅 Day 8 – 02-07-2026

## Topics Covered
- Passive Reconnaissance
- Active Reconnaissance

## Passive Reconnaissance
<img width="1736" height="781" alt="image" src="https://github.com/user-attachments/assets/d9a5d8a6-29f5-4469-b953-064e6462beb8" />

### Commands Learned

#### WHOIS

```bash
whois google.com
```
<img width="927" height="713" alt="Screenshot 2026-07-20 101920" src="https://github.com/user-attachments/assets/cab6f0d9-4b0c-48dc-a518-f7996d5f67b1" />

#### NSLOOKUP

```bash
nslookup google.com
```
<img width="892" height="815" alt="Screenshot 2026-07-20 102032" src="https://github.com/user-attachments/assets/71dfdbbc-6c3f-4510-954f-3309b92336a0" />

#### DIG

```bash
dig google.com
```
<img width="1681" height="617" alt="Screenshot 2026-07-20 092630" src="https://github.com/user-attachments/assets/8ab6c1f3-e7bc-4105-a8fe-823c14dc0205" />

<img width="1695" height="787" alt="image" src="https://github.com/user-attachments/assets/5a0a59ed-4b81-402e-841c-1f57ca5bb448" />

### Online Tools
- DNSDumpster
- Shodan

<img width="1693" height="902" alt="Screenshot 2026-07-20 092811" src="https://github.com/user-attachments/assets/3293e7bf-2ee9-412f-acbc-062b3ff43ad6" />



<img width="1655" height="832" alt="Screenshot 2026-07-20 092859" src="https://github.com/user-attachments/assets/6fb0592a-e85b-42d4-9575-7e35059327e1" />


## Active Reconnaissance

### Commands

#### ping

```bash
ping google.com
```

<img width="1392" height="657" alt="Screenshot 2026-07-20 093018" src="https://github.com/user-attachments/assets/d8436f13-cc61-45e4-93ef-07da5d4b3638" />

#### telnet

```bash
telnet example.com 80
```

#### netcat

```bash
nc example.com 80
```

## Skills Gained
- Domain Enumeration
- DNS Analysis
- Connectivity Testing
- Network Reconnaissance

## Screenshots
- AttackBox
- WHOIS Output
- NSLOOKUP Output
- DIG Output
- ping Result
- Netcat

---

# 📅 Day 9 – 03-07-2026

## Topics Covered
- Image Geolocation
- Reverse Image Search
- Missing Person Investigation

## What I Learned
- Learned how to identify image locations using visual clues.
- Practiced reverse image searching.
- Identified landmarks, buildings, road signs, and other clues.
- Completed the Missing Person investigation activity using OSINT techniques.

## Skills Gained
- Image Analysis
- Reverse Image Search
- Investigation Skills

## Screenshots
- Reverse Image Search
- Missing Person Activity

---

# 📅 Day 10 – 04-07-2026

## Topics Covered
- Networking Fundamentals

## What I Learned
- What is Networking
- How the Internet Works
- IP Address
- LAN (Local Area Network)
- LAN Topologies
- Subnetting Basics
- ARP (Address Resolution Protocol)
- DHCP (Dynamic Host Configuration Protocol)

## Skills Gained
- Networking Basics
- LAN Concepts
- Network Communication

## Screenshots
- Networking Notes
- LAN Diagram
- ARP Diagram
- DHCP Diagram

---

# 🛠️ Tools Used

- TryHackMe
- Kali Linux
- VirtualBox
- AttackBox
- WHOIS
- NSLOOKUP
- DIG
- DNSDumpster
- Shodan
- Netcat
- Telnet

---

# 📚 Skills Learned

- Cybersecurity Fundamentals
- TryHackMe
- Linux Basics
- OSINT
- Passive Reconnaissance
- Active Reconnaissance
- Networking Fundamentals
- Image Geolocation
- Reverse Image Search

---

## 🚀 Training Status

**Learning in Progress...**
